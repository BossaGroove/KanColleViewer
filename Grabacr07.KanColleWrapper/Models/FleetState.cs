	...
	public class FleetState : DisposableNotifier
	{
		...
		public FleetState(Homeport homeport, params Fleet[] fleets)
		{
			...
			this.CompositeDisposable.Add(new PropertyChangedWeakEventListener(KanColleClient.Current.Settings)
			{
                ...
                { "IsViewRangeCalcIncludeFirstFleet", (sender, args) => this.Calculate() },
                { "IsViewRangeCalcIncludeSecondFleet", (sender, args) => this.Calculate() },
			});
		}
		...
		internal void Update()
		{
			...
			if (heavilyDamaged)
			{
                if (!this.Situation.HasFlag(FleetSituation.HeavilyDamaged) && state.HasFlag(FleetSituation.Sortie) && this.HeavilyDamaged != null)
                {
                    this.HeavilyDamaged(this, new HeavilyDamagedEventArgs(this.Condition.Name));
                }
